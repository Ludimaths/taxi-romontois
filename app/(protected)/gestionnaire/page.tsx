...
const TABS: {id:TabId;icon:string;label:string}[] = [
  {id:"dashboard",   icon:"ðŸ ", label:"Tableau de bord"},
  {id:"rapport",     icon:"ðŸ“‹", label:"Rapport journalier"},
  {id:"imprevus",    icon:"âš¡", label:"ImprÃ©vus"},
  {id:"conducteurs", icon:"ðŸ‘¥", label:"Conducteurs"},
  {id:"vehicules",   icon:"ðŸšŒ", label:"VÃ©hicules"},
  {id:"circuits",    icon:"ðŸ›£ï¸", label:"Circuits"},
  {id:"incidents",   icon:"ðŸš¨", label:"Incidents"},
  {id:"alertes",     icon:"ðŸ””", label:"Alertes"},
  {id:"exports",     icon:"ðŸ“Š", label:"Exports"},
];
...
// â”€â”€ AssignModal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <Modal title={`Absence : ${driver.prenom} ${driver.nom}`} onClose={onClose}>
        ...
        <div style={{background:C.redL,borderRadius:12,padding:16,marginBottom:16,border:`1px solid #FCA5A5`}}>
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:C.gray600,marginTop:2}}>{driver.absence_motif || "Motif non renseignÃ©"}</div>
            </div>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          <InfoBox label="Circuit Ã  couvrir" value={circ ? `${circ.emoji} ${circ.nom} (${circ.num})` : "â€”"} highlight={C.red}/>
          <InfoBox label="VÃ©hicule habituel" value={driver.vehicule?.plaque || "â€”"}/>
          <InfoBox label="Ã‰cole" value={circ?.cercle?.nom || "â€”"}/>
          <InfoBox label="Enfants" value={circ ? `${circ.enfants_count} enfants` : "â€”"}/>
        </div>
        {circ
          ? <div style={{background:C.amberL,borderRadius:8,padding:12,marginBottom:16,fontSize:13,border:`1px solid #FDE68A`}}>
              âš  Circuit <strong>{circ.nom}</strong> non couvert. Assignez un remplaÃ§ant.
            </div>
          : <div style={{background:C.gray100,borderRadius:8,padding:12,marginBottom:16,fontSize:13}}>
              Ce conducteur n'a pas de circuit assignÃ©.
            </div>
        }
        <Btn full onClick={() => setStep("pick")} color={C.navyL}>Trouver un remplaÃ§ant â†’</Btn>
      </>}
      ...
      {step === "pick" && <>
          ...
          : avail.map(d => (
            <div key={d.id} onClick={() => setSel(d.id)}
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:12,color:C.gray400,marginTop:1}}>Permis {d.permis||"â€”"} Â· {d.tel||"â€”"}</div>
              </div>
        ...
        <div style={{marginTop:12}}>
          <Btn full onClick={doAssign} disabled={!sel || !circ || busy} color={C.green}>
            {busy ? "Attribution..." : "âœ… Attribuer le circuit"}
          </Btn>
        </div>
      </>}
      ...
      {step === "done" && (
        <div style={{textAlign:"center",padding:"32px 0"}}>
          ...
          <div style={{fontSize:18,fontWeight:800,color:C.green}}>Circuit attribuÃ© !</div>
          <div style={{fontSize:13,color:C.gray600,marginTop:8}}>Le conducteur remplaÃ§ant a Ã©tÃ© notifiÃ©.</div>
...
// â”€â”€ ChildAbsModal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <Modal title="Absence enfant" onClose={onClose}>
      <div style={{background:C.amberL,borderRadius:12,padding:16,marginBottom:16,border:`1px solid #FDE68A`}}>
        <div style={{fontSize:15,fontWeight:800,color:C.amber,marginBottom:4}}>
          âš  {child?.prenom} {child?.nom}
        </div>
        <div style={{fontSize:13,color:C.gray800}}>
          Motif : <strong>{absence.reason}</strong>
        </div>
        <div style={{fontSize:12,color:C.gray600,marginTop:4}}>
          SignalÃ© par {absence.reported_by} Ã  {fmtTime(absence.reported_at)}
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
        <InfoBox label="Circuit" value={circ ? `${circ.emoji} ${circ.nom}` : "â€”"}/>
        <InfoBox label="Ã‰cole" value={circ?.cercle?.nom || "â€”"}/>
        <InfoBox label="Conducteur" value={drv ? `${drv.prenom} ${drv.nom}` : "â€”"}/>
        <InfoBox label="Parent" value={child?.parent_nom || "â€”"}/>
        <InfoBox label="TÃ©l. parent" value={child?.parent_tel || "â€”"}/>
        <InfoBox label="Date absence" value={fmtDate(absence.date_absence)}/>
        {child?.adresse_mere && <InfoBox label="Adresse mÃ¨re" value={child.adresse_mere} full/>}
        {child?.adresse_pere && <InfoBox label="Adresse pÃ¨re" value={child.adresse_pere} full/>}
      </div>
      {!done
        ? <Btn full onClick={doTransmit} color={C.navyL} disabled={busy}>
            {busy ? "Envoi..." : `ðŸ“¨ Transmettre au conducteur${drv ? ` (${drv.prenom} ${drv.nom})` : ""}`}
          </Btn>
        : <div style={{textAlign:"center",padding:12,background:C.greenL,borderRadius:8,color:C.green,fontWeight:700,fontSize:13}}>
            âœ… Transmis au conducteur
          </div>
      }
    </Modal>
  );
...
// â”€â”€ DriverDetailModal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <Modal title={isNew ? "Nouveau conducteur" : `${driver!.prenom} ${driver!.nom}`} onClose={onClose} wide>
      {!edit && driver && (
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          <Btn small onClick={() => setEdit(true)} color={C.navyL}>âœï¸ Modifier</Btn>
          <Btn small onClick={() => window.print()} outline color={C.gray600}>ðŸ“„ Fiche PDF</Btn>
          <Btn small onClick={() => setDelConf(true)} outline color={C.red}>ðŸ—‘ï¸ Supprimer</Btn>
        </div>
      )}
      {delConf && (
        <div style={{background:C.redL,borderRadius:10,padding:14,marginBottom:16,border:`1px solid #FCA5A5`}}>
          <div style={{fontWeight:700,color:C.red,marginBottom:8}}>âš  Supprimer {driver?.prenom} {driver?.nom} ?</div>
      ...
      {edit ? (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <div><label style={labelSt}>PrÃ©nom</label><input style={inputSt} value={form.prenom} onChange={f("prenom")}/></div>
          <div><label style={labelSt}>Nom</label><input style={inputSt} value={form.nom} onChange={f("nom")}/></div>
          <div><label style={labelSt}>TÃ©lÃ©phone</label><input style={inputSt} value={form.tel} onChange={f("tel")}/></div>
          ...
          <div><label style={labelSt}>Permis (catÃ©gories)</label><input style={inputSt} value={form.permis} onChange={f("permis")} placeholder="B, D, D1"/></div>
          ...
          <div><label style={labelSt}>VÃ©hicule</label>
          ...
          <div style={{gridColumn:"1/-1",display:"flex",gap:8,marginTop:4}}>
            <Btn onClick={handleSave} color={C.green} disabled={busy}>{busy?"Sauvegarde...":"ðŸ’¾ Enregistrer"}</Btn>
          ...
          <div style={{display:"flex",gap:16,alignItems:"center",marginBottom:16}}>
            ...
            <div>
              ...
              <div style={{marginTop:4,display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                {driver!.tachygraphe && <Badge color="navy">ðŸ“‹ Tachygraphe</Badge>}
              </div>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
            <InfoBox label="TÃ©lÃ©phone"    value={driver!.tel||"â€”"}/>
            <InfoBox label="Permis"       value={driver!.permis||"â€”"}/>
            <InfoBox label="Exp. permis"  value={driver!.permis_exp?fmtDate(driver!.permis_exp):"â€”"}/>
            <InfoBox label="Affectation"  value={driver!.affectation}/>
            <InfoBox label="Circuit"      value={drvCirc ? `${drvCirc.emoji} ${drvCirc.nom}` : "â€”"}/>
            <InfoBox label="VÃ©hicule"     value={drvVeh?.plaque || "â€”"}/>
          ...
          {drvInc.length > 0 && (
            <div>
              <div style={{fontWeight:700,fontSize:11,color:C.gray600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>
                Incidents rÃ©cents ({drvInc.length})
              </div>
              {drvInc.slice(0,4).map(i => (
                <div key={i.id} style={{background:C.gray50,borderRadius:8,padding:"7px 12px",marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{fontSize:12}}>
                    <span style={{fontWeight:700}}>{fmtDate(i.reported_at)}</span> Â· {i.type} Â· {i.description.slice(0,60)}
                  </div>
                  <Badge color={i.status==="resolu"?"green":i.status==="en_cours"?"blue":"amber"}>
                    {i.status==="resolu"?"RÃ©solu":i.status==="en_cours"?"En cours":"Att."}
                  </Badge>
                </div>
              ))}
            </div>
...
// â”€â”€ VehicleDetailModal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function VehicleDetailModal({ vehicle, drivers, circuits, reparations, sb, onClose, onRefresh }: {
  ...
  const etatLabel: Record<string,string> = {bon:"En service",attention:"Ã€ surveiller",atelier:"En atelier"};
  ...
  return (
    <Modal title={isNew ? "Nouveau vÃ©hicule" : vehicle!.plaque} onClose={onClose} wide>
      {!edit && vehicle && (
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          <Btn small onClick={() => setEdit(true)} color={C.navyL}>âœï¸ Modifier</Btn>
          <Btn small onClick={() => window.print()} outline color={C.gray600}>ðŸ–¨ï¸ Imprimer QR</Btn>
          <Btn small onClick={() => setDelConf(true)} outline color={C.red}>ðŸ—‘ï¸ Supprimer</Btn>
        </div>
      )}
      {delConf && (
        <div style={{background:C.redL,borderRadius:10,padding:14,marginBottom:16,border:`1px solid #FCA5A5`}}>
          <div style={{fontWeight:700,color:C.red,marginBottom:8}}>âš  Supprimer {vehicle?.plaque} ?</div>
          <div style={{display:"flex",gap:8}}>
            <Btn small onClick={handleDelete} color={C.red} disabled={busy}>Confirmer</Btn>
            <Btn small onClick={() => setDelConf(false)} outline color={C.gray600}>Annuler</Btn>
          </div>
      ...
      {edit ? (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {isNew && <div><label style={labelSt}>ID vÃ©hicule</label><input style={inputSt} value={form.id} onChange={f("id")} placeholder="FR-123456"/></div>}
          ...
          <div><label style={labelSt}>ModÃ¨le</label><input style={inputSt} value={form.modele} onChange={f("modele")}/></div>
          ...
          <div><label style={labelSt}>KilomÃ©trage</label><input type="number" style={inputSt} value={form.km} onChange={f("km")}/></div>
          <div style={{gridColumn:"1/-1",display:"flex",gap:8,marginTop:4}}>
            <Btn onClick={handleSave} color={C.green} disabled={busy}>{busy?"Sauvegarde...":"ðŸ’¾ Enregistrer"}</Btn>
            {!isNew && <Btn onClick={() => setEdit(false)} outline color={C.gray600}>Annuler</Btn>}
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr",gap:20}}>
          <div>
            <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:40}}>ðŸšŒ</div>
              <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
              ...
              <InfoBox label="KilomÃ©trage" value={`${vehicle!.km?.toLocaleString("fr-CH")} km`}/>
              <InfoBox label="CT" value={vehicle!.ct_date||"â€”"}/>
              <InfoBox label="Assurance" value={vehicle!.assurance_date||"â€”"}/>
              <InfoBox label="Circuit" value={vehCirc ? `${vehCirc.emoji} ${vehCirc.nom}` : "â€”"}/>
              <InfoBox label="Conducteur habituel" value={habDriver ? `${habDriver.prenom} ${habDriver.nom}` : "â€”"}/>
            </div>
            {vehRep.length > 0 && (
              <div>
                <div style={{fontWeight:700,fontSize:11,color:C.gray600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>
                  RÃ©parations ({vehRep.length})
                </div>
                {vehRep.slice(0,4).map(r => (
                  <div key={r.id} style={{background:C.gray50,borderRadius:8,padding:"7px 12px",marginBottom:5,fontSize:12}}>
                    ...
                    {" Â· "}{r.description.slice(0,60)}
                    {r.cout ? <span style={{color:C.amber,fontWeight:700}}> Â· {r.cout} CHF</span> : null}
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{background:C.gray50,borderRadius:12,padding:16,textAlign:"center"}}>
            <div style={{fontSize:13,fontWeight:700,color:C.gray600,marginBottom:12}}>QR Code vÃ©hicule</div>
            <div style={{background:C.white,borderRadius:8,padding:16,marginBottom:12,border:`1px solid ${C.gray200}`}}>
              <div style={{fontSize:40,marginBottom:6}}>ðŸ“±</div>
              <div style={{fontSize:10,color:C.gray600,wordBreak:"break-all",marginBottom:4}}>{scanUrl}</div>
              <div style={{fontSize:10,color:C.gray400,fontFamily:"monospace"}}>{vehicle!.qr_token?.slice(0,12)}â€¦</div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              <Btn small onClick={doCopy} color={C.navyL}>{copied?"âœ… CopiÃ© !":"ðŸ“‹ Copier l'URL"}</Btn>
              <Btn small onClick={() => window.open(scanUrl,"_blank")} outline color={C.gray600}>ðŸ”— Ouvrir la page</Btn>
              <Btn small onClick={() => window.print()} outline color={C.gray600}>ðŸ–¨ï¸ Imprimer</Btn>
            </div>
          </div>
        </div>
      )}
...
// â”€â”€ CircuitDetailModal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function CircuitDetailModal({ circuit, drivers, enfants, sb, onClose, onRefresh }: {
  ...
  return (
    <Modal title={isNew ? "Nouveau circuit" : `${circuit!.emoji} ${circuit!.nom}`} onClose={onClose} wide>
      ...
      {delConf && (
        <div style={{background:C.redL,borderRadius:10,padding:14,marginBottom:16,border:`1px solid #FCA5A5`}}>
          <div style={{fontWeight:700,color:C.red,marginBottom:8}}>âš  Supprimer le circuit {circuit?.nom} ?</div>
          <div style={{display:"flex",gap:8}}>
            <Btn small onClick={handleDelete} color={C.red} disabled={busy}>Confirmer</Btn>
            <Btn small onClick={() => setDelConf(false)} outline color={C.gray600}>Annuler</Btn>
          </div>
      ...
      {edit ? (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div><label style={labelSt}>NumÃ©ro</label><input style={inputSt} value={form.num} onChange={f("num")} placeholder="01"/></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
          <div>
            ...
            <div style={{fontSize:13,color:C.gray600,marginBottom:16}}>
              Circuit NÂ°{circuit!.num} Â· {circuit!.cercle?.nom||"â€”"}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
              <InfoBox label="Enfants inscrits" value={circuit!.enfants_count}/>
              <InfoBox label="Distance aller" value={circuit!.km_aller ? `${circuit!.km_aller} km` : "â€”"}/>
            </div>
            <div style={{fontWeight:700,fontSize:11,color:C.gray600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>
              Conducteur(s)
            </div>
            {circDrivers.length === 0
              ? <div style={{color:C.red,fontSize:13,fontWeight:700}}>âš  Aucun conducteur assignÃ©</div>
              : circDrivers.map(d => (
                <div key={d.id} style={{display:"flex",gap:10,alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.gray100}`}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:11,color:C.gray400}}>{d.tel||"â€”"}</div>
                  </div>
...
// â”€â”€ ImprevuModal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function ImprevuModal({ drivers, sb, onClose, onRefresh }: {
  ...
  const TYPES = [{v:"absence",l:"Absence"},{v:"ecole",l:"Ã‰cole"},{v:"parent",l:"Parent"},
    {v:"vehicule",l:"VÃ©hicule"},{v:"meteo",l:"MÃ©tÃ©o"},{v:"autre",l:"Autre"}];
  ...
  return (
    <Modal title="CrÃ©er un imprÃ©vu" onClose={onClose} wide>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        <div>
          <div style={{fontWeight:700,fontSize:11,color:C.gray600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>
            Type d'imprÃ©vu
          ...
          <textarea value={msg} onChange={e => setMsg(e.target.value)} rows={5}
            ...
            placeholder="DÃ©crivez l'imprÃ©vu en dÃ©tail..."/>
          <Btn full onClick={handleSend} color={C.navyL} disabled={busy || !msg.trim()}>
            {busy ? "Envoi..." : "ðŸ“¤ Envoyer la notification"}
          </Btn>
        </div>
        <div>
          <div style={{fontWeight:700,fontSize:11,color:C.gray600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:12}}>
            {([["all","Tous"],["conducteur","Conducteur"],["mecanicien","MÃ©canicien"],["admin","Admin"]] as const).map(([v,l]) => (
          ...
          {target !== "conducteur" && (
            <div style={{background:C.amberL,borderRadius:8,padding:12,fontSize:13,color:C.amber,fontWeight:600}}>
              âš  Notification envoyÃ©e Ã {" "}
              {target==="all"?"tous les utilisateurs":target==="mecanicien"?"tous les mÃ©caniciens":"tous les administrateurs"}.
            </div>
          )}
        </div>
      </div>
...
// â”€â”€ TabDashboard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:22}}>
        ...
        <div style={cardStyle(C.green)} onClick={() => setTab("vehicules")}>
          <div style={{fontSize:22,marginBottom:4}}>ðŸšŒ</div>
          ...
          <div style={{fontSize:12,color:C.gray600,marginTop:2}}>VÃ©hicules en service</div>
        ...
        <div style={cardStyle(C.amber)} onClick={() => setTab("vehicules")}>
          ...
          <div style={{fontSize:12,color:C.gray600,marginTop:2}}>En rÃ©paration</div>
          <div style={{fontSize:11,color:C.gray400,marginTop:2}}>{activeRep.length} rÃ©paration(s) en cours</div>
        </div>
        {/* 3 */}
        <div style={cardStyle(C.navyL)} onClick={() => setTab("conducteurs")}>
          ...
          <div style={{fontSize:12,color:C.gray600,marginTop:2}}>Conducteurs prÃ©sents</div>
        ...
        <div style={cardStyle(C.red)} onClick={() => setTab("conducteurs")}>
          <div style={{fontSize:22,marginBottom:4}}>âš ï¸</div>
          ...
          <div style={{fontSize:11,color:C.gray400,marginTop:2}}>{absents.filter(d=>!!d.circuit_id).length} circuits Ã  couvrir</div>
        </div>
        {/* 5 */}
        <div style={cardStyle(C.red)} onClick={() => setTab("incidents")}>
          <div style={{fontSize:22,marginBottom:4}}>ðŸš¨</div>
          ...
          <div style={{fontSize:11,color:C.gray400,marginTop:2}}>{incidents.filter(i=>i.status==="resolu").length} rÃ©solus</div>
        </div>
        {/* 6 */}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr",gap:18}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          {/* Section 2 â€” Circuits du jour */}
          <Card>
            <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.gray200}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontWeight:700,color:C.gray800}}>ðŸ›£ï¸ Circuits du jour</span>
              <button onClick={() => setTab("circuits")} style={{background:"none",border:"none",color:C.navyL,fontSize:12,fontWeight:700,cursor:"pointer"}}>Voir tout â†’</button>
            </div>
                          ...
                          {drv ? `${drv.prenom} ${drv.nom}` :
                            absDrv
                              ? <button onClick={() => onOpenAbsent(absDrv)}
                                  ...
                                  âš  Non couvert â€” gÃ©rer
                                </button>
                              : "â€”"
                          } Â· {circ.enfants_count} enfants
                        </div>
                      </div>
                    </div>
          ...
          {/* Section 1 â€” Absences conducteurs gÃ©rÃ©es */}
          {absencesCond.length > 0 && (
            <Card>
              <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.gray200}`}}>
                <span style={{fontWeight:700,color:C.gray800}}>ðŸ‘¥ Absences gÃ©rÃ©es</span>
              </div>
                  ...
                  <div key={ab.id} style={{padding:"10px 16px",borderBottom:`1px solid ${C.gray100}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:700,color:C.gray800}}>
                        ...
                        {replDrv && <span style={{color:C.green,fontWeight:600}}> â†’ remplacÃ© par {replDrv.prenom} {replDrv.nom}</span>}
                      </div>
                      <div style={{fontSize:11,color:C.gray400,marginTop:2}}>
                        {abCirc?.emoji} {abCirc?.nom} Â· {ab.motif||"â€”"}
                      </div>
                    </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <Card>
            ...
            {todayAbs.length === 0
              ? <div style={{padding:20,textAlign:"center",color:C.gray400,fontSize:13}}>âœ… Aucune absence aujourd'hui</div>
                  ...
                  <div key={a.id} onClick={() => onOpenChildAbs(a)}
                    ...
                    <div>
                      ...
                      <div style={{fontSize:11,color:C.gray400,marginTop:1}}>
                        {a.reason} Â· {circ?.emoji} {circ?.nom} Â· {fmtTime(a.reported_at)}
                      </div>
                    </div>
                    <Badge color={a.transmitted_to_driver?"green":"amber"}>
                      {a.transmitted_to_driver?"Transmis":"Ã€ transmettre"}
                    </Badge>
                  </div>
                );
              })
          ...
          {/* Section 3 â€” Incidents actifs */}
          {openInc.length > 0 && (
            <Card>
              <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.gray200}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontWeight:700,color:C.red}}>âš¡ Incidents actifs</span>
                <button onClick={() => setTab("incidents")} style={{background:"none",border:"none",color:C.navyL,fontSize:12,fontWeight:700,cursor:"pointer"}}>GÃ©rer â†’</button>
              </div>
              {openInc.slice(0,4).map(i => (
                <div key={i.id} onClick={() => onOpenInc(i)}
                  ...
                  <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                    <Badge color={i.status==="en_cours"?"blue":"amber"}>{i.status==="en_cours"?"En cours":"Ã€ traiter"}</Badge>
                    <span style={{fontSize:11,color:C.gray400}}>{i.conducteur?.prenom} {i.conducteur?.nom} Â· {fmtTime(i.reported_at)}</span>
                  </div>
                </div>
              ))}
            </Card>
          ...
          {/* Section 4 â€” RÃ©parations en cours */}
          {activeRep.length > 0 && (
            <Card>
              <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.gray200}`}}>
                <span style={{fontWeight:700,color:C.gray800}}>ðŸ”§ RÃ©parations en cours</span>
              </div>
...
// â”€â”€ TabRapport â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TabRapport({ drivers, incidents, absences, alerts, serviceLogs, sb, onRefresh }: {
  ...
  const sendReport = async () => {
    ...
    await sb.from("alertes").insert({
      ...
      message:`Rapport ${period==="today"?"du jour":period==="yesterday"?"d'hier":period==="week"?"de la semaine":"du mois"} : ${filtInc.length} incidents Â· ${filtAbs.length} absences enfants Â· ${filtLogs.length} services Â· ${filtAlt.length} alertes.`,
      read: false,
    });
    setSending(false);
    onRefresh();
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...filtAbs.map(a  => [fmtDate(a.date_absence),"Absence enfant","â€”",a.reason,a.transmitted_to_driver?"transmis":"en attente",""]),
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        <Stat label="Incidents" value={filtInc.length} sub={`${filtInc.filter(i=>i.status==="resolu").length} rÃ©solus`} icon="ðŸš¨" color={C.red}/>
        <Stat label="Absences enfants" value={filtAbs.length} sub={`${filtAbs.filter(a=>a.transmitted_to_driver).length} transmises`} icon="ðŸ‘¶" color={C.amber}/>
        <Stat label="Services" value={filtLogs.length} sub={`${filtLogs.filter(l=>l.is_replacement).length} remplacements`} icon="ðŸšŒ" color={C.navyL}/>
        <Stat label="Alertes" value={filtAlt.length} sub={`${filtAlt.filter(a=>!a.read).length} non lues`} icon="ðŸ””" color={C.amber}/>
      </div>
      {/* Actions */}
      <div style={{display:"flex",gap:8,marginBottom:20}}>
        <Btn onClick={sendReport} color={C.navyL} disabled={sending}>{sending?"Envoi...":"ðŸ“¨ Envoyer Ã  l'admin"}</Btn>
        <Btn onClick={exportCSV} outline color={C.gray600}>ðŸ“Š TÃ©lÃ©charger CSV</Btn>
      </div>
      {/* Timeline */}
      <Card>
        ...
        <div style={{maxHeight:480,overflowY:"auto"}}>
          {[
            ...filtInc.map(i => ({t:i.reported_at,icon:"ðŸš¨",label:`Incident ${i.type}`,detail:i.description.slice(0,80),status:i.status,actor:i.conducteur?`${i.conducteur.prenom} ${i.conducteur.nom}`:""})),
            ...filtAbs.map(a => {const child=a.enfant;return{t:a.reported_at,icon:"ðŸ‘¶",label:"Absence enfant",detail:`${child?.prenom||""} ${child?.nom||""} â€” ${a.reason}`,status:a.transmitted_to_driver?"transmis":"en attente",actor:a.reported_by};}),
            ...filtLogs.map(l => ({t:l.created_at,icon:"ðŸšŒ",label:l.is_replacement?"Remplacement":"Service",detail:l.conducteur?`${l.conducteur.prenom} ${l.conducteur.nom}`:"",status:l.status,actor:""})),
          ...
          {filtInc.length + filtAbs.length + filtLogs.length === 0 && (
            <div style={{padding:32,textAlign:"center",color:C.gray400}}>Aucun Ã©vÃ©nement pour cette pÃ©riode</div>
          )}
        </div>
      </Card>
    </div>
...
// â”€â”€ TabImprevus â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TabImprevus({ alerts, drivers, sb, onRefresh }: {
  alerts: Alerte[]; drivers: Conducteur[]; sb: SB; onRefresh: () => void;
}) {
  const [open, setOpen] = useState(false);
  const imprevus = alerts.filter(a => a.type === "imprÃ©vu");
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div style={{fontWeight:800,fontSize:18,color:C.gray800}}>âš¡ ImprÃ©vus</div>
        <Btn onClick={() => setOpen(true)} color={C.navyL}>+ CrÃ©er un imprÃ©vu</Btn>
      </div>
        ...
        {imprevus.length === 0
          ...
          : imprevus.map(a => (
            <div key={a.id} style={{padding:"12px 16px",borderBottom:`1px solid ${C.gray100}`,
              ...
              {!a.read && (
                <Btn small onClick={() => markRead(a.id)} color={C.green}>âœ“ Lu</Btn>
              )}
            </div>
          ))
        }
...
// â”€â”€ TabConducteurs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TabConducteurs({ drivers, circuits, vehicles, incidents, sb, onRefresh }: {
  ...
  const exportCSV = () => {
    downloadCSV(`conducteurs_${isoToday()}.csv`,
      ...
      ["PrÃ©nom","Nom","TÃ©l","Permis","Exp. permis","Circuit","VÃ©hicule","Statut","Affectation"]
    );
  };
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
        ...
        <Btn onClick={exportCSV} outline color={C.gray600}>ðŸ“Š Export CSV</Btn>
      </div>
      <div style={{fontSize:12,color:C.gray400,marginBottom:12}}>
        {filtered.length} conducteur(s) Â· {drivers.filter(d=>d.status==="absent").length} absent(s) Â· {drivers.filter(d=>d.status==="en_service").length} en service
      </div>
      <Card>
        {filtered.map(d => {
          ...
          return (
            <div key={d.id} onClick={() => { setAddOpen(false); setSelDrv(d); }}
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:C.gray400,marginTop:1}}>
                  {circ ? `${circ.emoji} ${circ.nom}` : "Pas de circuit"} Â· {d.vehicule?.plaque||d.vehicule_id||"â€”"} Â· {d.tel||"â€”"}
                </div>
        ...
        {filtered.length === 0 && (
          <div style={{padding:32,textAlign:"center",color:C.gray400}}>Aucun conducteur trouvÃ©</div>
        )}
      </Card>
    </div>
  );
...
// â”€â”€ TabVehicules â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TabVehicules({ vehicles, drivers, circuits, reparations, sb, onRefresh }: {
  ...
  const exportCSV = () => {
    downloadCSV(`vehicules_${isoToday()}.csv`,
      ...
      ["Plaque","Marque","ModÃ¨le","Places","Handi","Ã‰tat","CT","Assurance","Km"]
    );
  };
  ...
  return (
    <div>
          ...
          return (
            <div key={v.id} onClick={() => { setAddOpen(false); setSelVeh(v); }}
              ...
              <div style={{fontSize:12,color:C.gray400}}>
                <div>ðŸª‘ {v.places} places{v.places_handi>0?` + ${v.places_handi} handi`:""}</div>
                <div>ðŸ“ {circ ? `${circ.emoji} ${circ.nom}` : "Pas de circuit"}</div>
                <div>ðŸ‘¤ {drv ? `${drv.prenom} ${drv.nom}` : "â€”"}</div>
                <div>ðŸ›£ï¸ {v.km?.toLocaleString("fr-CH")} km</div>
              </div>
            </div>
          );
        })}
      ...
      {filtered.length === 0 && (
        <div style={{padding:40,textAlign:"center",color:C.gray400}}>Aucun vÃ©hicule trouvÃ©</div>
      )}
    </div>
  );
}

// â”€â”€ TabCircuits â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TabCircuits({ circuits, drivers, enfants, sb, onRefresh }: {
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
        <input value={search} onChange={e => setSearch(e.target.value)}
          style={{...inputSt,flex:1,minWidth:200}} placeholder="Rechercher circuit, Ã©cole..."/>
        <Btn onClick={() => setAddOpen(true)} color={C.green}>+ Ajouter</Btn>
      </div>
      <div style={{fontSize:12,color:C.gray400,marginBottom:12}}>
        {circuits.length} circuits Â· {circuits.filter(c=>!!drivers.find(d=>d.circuit_id===c.id&&d.status!=="absent")).length} couverts
      </div>
      <Card>
        {filtered.map(circ => {
          const drv  = drivers.find(d => d.circuit_id === circ.id && d.status !== "absent");
          ...
          return (
            <div key={circ.id} onClick={() => { setAddOpen(false); setSelCirc(circ); }}
              ...
              <div style={{flex:1}}>
                <div style={{fontWeight:700,fontSize:14,color:C.gray800}}>{circ.num} â€” {circ.nom}</div>
                <div style={{fontSize:12,color:C.gray600,marginTop:1}}>
                  {circ.cercle?.nom||"â€”"} Â· {circ.enfants_count} enfants Â· {circ.km_aller} km
                </div>
                <div style={{fontSize:11,color:drv?C.gray400:C.red,marginTop:1}}>
                  ðŸ‘¤ {drv ? `${drv.prenom} ${drv.nom}` : absDrv ? `âš  ${absDrv.prenom} ${absDrv.nom} (absent)` : "Non assignÃ©"}
                </div>
              </div>
              <Badge color={drv?(drv.status==="en_service"?"green":"amber"):(!drv&&absDrv)?"red":"gray"}>
                {drv?statusLabel(drv.status):(!drv&&absDrv)?"Non couvert":"â€”"}
              </Badge>
            </div>
          );
        })}
...
// â”€â”€ TabIncidents â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TabIncidents({ incidents, drivers, vehicles, circuits, enfants, sb, onRefresh }: {
  ...
  const handleAction = async (id: number, response: string, status: "en_cours"|"resolu", extra?: string) => {
    ...
    if (extra === "transmis_meca") {
      ...
      await sb.from("alertes").insert({type:"transmis_meca",severity:"haute",
        message:`Incident transmis au mÃ©canicien : ${inc?.vehicule_id||""} â€” ${inc?.description?.slice(0,100)||""}`,
  ...
  const exportCSV = () => {
    downloadCSV(`incidents_${isoToday()}.csv`,
      ...
      ["Date","Type","Description","Statut","Conducteur","VÃ©hicule","Circuit"]
    );
  };

  const TYPE_LABEL: Record<string,string> = {
    panne:"ðŸ”§ Panne",voyant:"ðŸ’¡ Voyant",accident:"ðŸš¨ Accident",retard:"â° Retard",
    degradation:"ðŸªŸ DÃ©gradation",enfant:"ðŸ‘¶ Enfant",parent:"ðŸ‘¨â€ðŸ‘©â€ðŸ‘§ Parent",autre:"â“ Autre",
  };
  ...
  return (
    <div>
      ...
      {addOpen && (
        <Modal title="CrÃ©er un incident" onClose={() => setAddOpen(false)}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <Btn full onClick={handleCreate} color={C.red} disabled={saving||!newInc.description.trim()}>
              {saving?"CrÃ©ation...":"ðŸš¨ CrÃ©er l'incident"}
            </Btn>
          </div>
        </Modal>
      )}
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {(["today","week","month","year"] as const).map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              ...
              {p==="today"?"Aujourd'hui":p==="week"?"Semaine":p==="month"?"Mois":"AnnÃ©e"}
            </button>
          ))}
          ...
          {(["all","en_attente","en_cours","resolu"] as const).map(s => (
            <button key={s} onClick={() => setStatusF(s)}
              style={{padding:"6px 14px",borderRadius:8,border:`2px solid ${statusF===s?C.navyL:C.gray200}`,
                ...
                fontWeight:700,fontSize:12,cursor:"pointer"}}>
              {s==="all"?"Tous":s==="en_attente"?"En attente":s==="en_cours"?"En cours":"RÃ©solus"}
            </button>
          ))}
        </div>
        <div style={{display:"flex",gap:8}}>
          <Btn small onClick={() => setAddOpen(true)} color={C.red}>+ CrÃ©er</Btn>
          <Btn small onClick={exportCSV} outline color={C.gray600}>ðŸ“Š CSV</Btn>
        </div>
      </div>
      ...
      <Card>
        {filtered.length === 0
          ? <div style={{padding:40,textAlign:"center",color:C.gray400}}>âœ… Aucun incident</div>
          : filtered.map(i => (
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:4,flexWrap:"wrap"}}>
                    ...
                    <Badge color={i.status==="resolu"?"green":i.status==="en_cours"?"blue":"amber"}>
                      {i.status==="resolu"?"RÃ©solu":i.status==="en_cours"?"En cours":"En attente"}
                    </Badge>
                  </div>
                  <div style={{fontSize:12,color:C.gray600,marginBottom:2}}>{i.description.slice(0,100)}</div>
                  <div style={{fontSize:11,color:C.gray400}}>
                    {i.conducteur?.prenom} {i.conducteur?.nom} Â· {i.vehicule?.plaque} Â· {i.circuit?.nom} Â· {fmtDT(i.reported_at)}
                  </div>
                  {i.response && (
                    <div style={{fontSize:11,color:C.green,marginTop:4,fontWeight:600}}>
                      ðŸ’¬ {i.response.slice(0,80)}
                    </div>
                  )}
                </div>
                <div style={{fontSize:18,color:C.gray400}}>â†’</div>
              </div>
            </div>
          ))
        }
...
// â”€â”€ TabAlertes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TabAlertes({ alerts, sb, onRefresh }: {
  ...
  const TYPE_ICON: Record<string,string> = {
    document:"ðŸ“„",vehicule:"ðŸšŒ",conducteur:"ðŸ‘¤",reparation:"ðŸ”§",
    rapport_admin:"ðŸ“‹","imprÃ©vu":"âš¡",transmis_meca:"ðŸ”§",absence:"ðŸ‘¶",
  };
        ...
        <Card>
          {items.map(a => (
            <div key={a.id} style={{display:"flex",gap:12,alignItems:"flex-start",padding:"12px 16px",
              ...
              <div style={{fontSize:22,flexShrink:0}}>{TYPE_ICON[a.type]||"ðŸ””"}</div>
              <div style={{flex:1}}>
                <div style={{fontWeight:700,fontSize:13,color:C.gray800}}>{a.message}</div>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
                ...
                {!a.read && <Btn small onClick={() => markRead(a.id)} color={C.green}>âœ“ Lu</Btn>}
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
        <div style={{fontWeight:800,fontSize:18,color:C.gray800}}>
          ðŸ”” Alertes {alerts.filter(a=>!a.read).length > 0 && (
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          {alerts.some(a => !a.read) && (
            <Btn small onClick={markAllRead} color={C.green}>âœ“ Tout marquer lu</Btn>
          )}
        </div>
      </div>
      {displayed.length === 0
        ? <div style={{padding:48,textAlign:"center",color:C.gray400}}>
            <div style={{fontSize:48,marginBottom:12}}>âœ…</div>
            Toutes les alertes sont traitÃ©es
          </div>
        : <>
          <Section title="Critique" items={critique} color={C.red}/>
          <Section title="Haute prioritÃ©" items={haute}    color={C.amber}/>
...
// â”€â”€ TabExports â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TabExports({ drivers, vehicles, circuits, incidents, serviceLogs }: {
  ...
  return (
    <div>
      <div style={{fontWeight:800,fontSize:18,color:C.gray800,marginBottom:16}}>ðŸ“Š Exports</div>

      <ExportSection title="ðŸ‘¥ Conducteurs">
        <Btn onClick={() => downloadCSV(`conducteurs_${today}.csv`,
          ...
          ["PrÃ©nom","Nom","TÃ©l","Permis","Exp. permis","Circuit","VÃ©hicule","Statut","Affectation","Tachygraphe"]
        )} color={C.navyL}>ðŸ“Š Excel (CSV)</Btn>
        <Btn onClick={() => window.print()} outline color={C.gray600}>ðŸ“„ PDF (impression)</Btn>
      </ExportSection>

      <ExportSection title="ðŸšŒ VÃ©hicules">
        <Btn onClick={() => downloadCSV(`vehicules_${today}.csv`,
          ...
          ["Plaque","Marque","ModÃ¨le","Places","Handi","Ã‰tat","CT","Assurance","Km","Circuit"]
        )} color={C.navyL}>ðŸ“Š Excel (CSV)</Btn>
        <Btn onClick={() => window.print()} outline color={C.gray600}>ðŸ“„ PDF QR codes</Btn>
      </ExportSection>

      <ExportSection title="ðŸ›£ï¸ Circuits">
        <Btn onClick={() => downloadCSV(`circuits_${today}.csv`,
          ...
          ["NÂ°","Nom","Emoji","Ã‰cole","Enfants","Km aller","Conducteur"]
        )} color={C.navyL}>ðŸ“Š Excel (CSV)</Btn>
      </ExportSection>

      <ExportSection title="ðŸš¨ Incidents">
        <Btn onClick={() => downloadCSV(`incidents_30j_${today}.csv`,
          ...
          ["Date","Type","Description","Statut","Conducteur","VÃ©hicule","Circuit","RÃ©ponse"]
        )} color={C.navyL}>ðŸ“Š 30 derniers jours</Btn>
      ...
      <ExportSection title="ðŸ“‹ Services">
        <Btn onClick={() => downloadCSV(`services_${today}.csv`,
          ...
          ["Date","PrÃ©nom","Nom","Circuit","DÃ©but","Fin","Statut","Remplacement","Remplace"]
        )} color={C.navyL}>ðŸ“Š Historique services</Btn>
      </ExportSection>
    </div>
  );
}
...
export default function GestionnaireDashboard() {
  ...
  // â”€â”€ Realtime â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  useEffect(() => {
    fetchAll();
  ...
  // â”€â”€ Computed badges â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const openInc    = incidents.filter(i => i.status !== "resolu");
  const unread     = alerts.filter(a => !a.read);
  const absents    = drivers.filter(d => d.status === "absent");
  const atelierVeh = vehicles.filter(v => (v.etat as string) === "atelier").length;
  ...
  const BADGES: Record<TabId,number> = {
    ...
    imprevus:   alerts.filter(a=>!a.read&&a.type==="imprÃ©vu").length,
  ...
  if (loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"60vh",flexDirection:"column",gap:12,color:C.gray400}}>
      ...
      <div style={{fontWeight:700,fontSize:15}}>Chargement du tableau de bordâ€¦</div>
    </div>
  );
  ...
  return (
    <div style={{maxWidth:1400,margin:"0 auto"}}>
      ...
      {/* â”€â”€ Global modals â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {absentModal && (
      ...
      <div style={{background:`linear-gradient(135deg,${C.navy},${C.navyL})`,borderRadius:16,padding:"22px 28px",
        ...
        <div>
          ...
          <div style={{fontSize:13,opacity:0.8,marginBottom:6}}>
            Plateforme de gestion Taxi Romontois Â· Vue temps rÃ©el
          </div>
      ...
      {/* â”€â”€ PC tab bar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      ...
      {/* â”€â”€ Tab content â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {tab === "dashboard" && (
        <TabDashboard
...
// â”€â”€ IncidentActionModal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function IncidentActionModal({ incident, drivers, vehicles, circuits, enfants, onClose, onAction }: {
  ...
  const TYPE_LABEL: Record<string,string> = {
    panne:"ðŸ”§ Panne vÃ©hicule", voyant:"ðŸ’¡ Voyant moteur", accident:"ðŸš¨ Accident",
    retard:"â° Retard", degradation:"ðŸªŸ DÃ©gradation", enfant:"ðŸ‘¶ ProblÃ¨me enfant",
    parent:"ðŸ‘¨â€ðŸ‘©â€ðŸ‘§ ProblÃ¨me parent", autre:"â“ Autre",
  };
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        ...
        <div>
          ...
          <Badge color={incident.status==="resolu"?"green":incident.status==="en_cours"?"blue":"amber"}>
            {incident.status==="resolu"?"RÃ©solu":incident.status==="en_cours"?"En cours":"En attente"}
          </Badge>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <InfoBox label="SignalÃ© le"   value={fmtDT(incident.reported_at)}/>
            <InfoBox label="Conducteur"   value={drv ? `${drv.prenom} ${drv.nom}` : "â€”"}/>
            <InfoBox label="VÃ©hicule"     value={veh?.plaque || "â€”"}/>
            <InfoBox label="Circuit"      value={circ ? `${circ.emoji} ${circ.nom}` : "â€”"}/>
            {drv?.tel && <InfoBox label="TÃ©l. conducteur" value={drv.tel} full/>}
          </div>
            ...
            <div style={{marginTop:12}}>
              ...
              {circEnfants.slice(0, 5).map(e => (
                <div key={e.id} style={{background:C.white,border:`1px solid ${C.gray200}`,borderRadius:8,padding:"6px 10px",marginBottom:5}}>
                  ...
                  {e.parent_nom && <div style={{fontSize:11,color:C.gray600}}>{e.parent_nom} Â· {e.parent_tel || "â€”"}</div>}
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
            {isPanne && <>
              <Btn onClick={() => quickAction("Incident transmis au mÃ©canicien.","transmis_meca")} small color={C.navyL} disabled={busy}>
                ðŸ”§ Envoyer au mÃ©canicien
              </Btn>
              <Btn onClick={() => quickAction(`VÃ©hicule ${veh?.plaque||""} immobilisÃ© en atelier.`,"immobiliser")} small color={C.red} disabled={busy}>
                ðŸš« Immobiliser le vÃ©hicule
              </Btn>
            </>}
            {isRetard && <>
              <Btn onClick={() => quickAction(`Ã‰cole ${circ?.cercle?.nom||""} informÃ©e du retard.`)} small color={C.amber} disabled={busy}>
                ðŸ« Informer l'Ã©cole
              </Btn>
              <Btn onClick={() => quickAction(`Parents du circuit ${circ?.nom||""} informÃ©s du retard.`,"informer_parents")} small color={C.navyL} disabled={busy}>
                ðŸ‘¨â€ðŸ‘©â€ðŸ‘§ Informer les parents
              </Btn>
            </>}
            {isPersonne && <>
              <Btn onClick={() => quickAction(`Parent contactÃ© â€” circuit ${circ?.nom||""}.`)} small color={C.amber} disabled={busy}>
                ðŸ“ž Contacter le parent
              </Btn>
              <Btn onClick={() => quickAction(`Ã‰cole ${circ?.cercle?.nom||""} contactÃ©e.`)} small color={C.navyL} disabled={busy}>
                ðŸ« Contacter l'Ã©cole
              </Btn>
            </>}
          </div>

          <div style={{marginBottom:10}}>
            <label style={labelSt}>Commentaire / rÃ©ponse au conducteur</label>
            <textarea value={response} onChange={e => setResponse(e.target.value)} rows={4}
              style={{...inputSt,resize:"vertical"}}
              placeholder="Votre rÃ©ponse visible par le conducteur..."/>
          </div>
              ...
              {(["en_cours","resolu"] as const).map(s => (
                <button key={s} onClick={() => setStatus(s)}
                  style={{flex:1,padding:"8px 10px",borderRadius:8,
                    ...
                    fontWeight:700,fontSize:12,cursor:"pointer"}}>
                  {s === "resolu" ? "âœ… RÃ©solu" : "ðŸ”„ En cours"}
                </button>
              ))}
            </div>
          </div>
          ...
          <Btn full onClick={handleSave} color={status==="resolu"?C.green:C.navyL} disabled={busy}>
            {busy ? "Sauvegarde..." : status==="resolu" ? "âœ… RÃ©soudre l'incident" : "ðŸ’¾ Enregistrer"}
          </Btn>
        </div>
      </div>
    </Modal>
